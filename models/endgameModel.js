...
/*class EndGame {
  ...
  static async endGame(game) {
    try {
      ...
      // For this template both are  tied (id = 1) and with one point
     // winner and loser
     let playerChiefCard = await pool.query("select * from user_game_card where ugc_crd_type_id = 1 and ugc_user_game_id = ?", [game.player.id]);
     let oppChiefCard = await pool.query("select * from user_game_card where ugc_crd_type_id = 1 and ugc_user_game_id = ?", [game.opponents[0].id]);
     let sqlScore = `Insert into scoreboard (sb_ug_id,sb_state_id) values (?,?)`;
     if(playerChiefCard.ugc_card_health <= 0){
      await pool.query(sqlScore, [game.player.id, 3]);
      await pool.query(sqlScore, [game.opponents[0].id, 2]);
     }else if(oppChiefCard.ugc_card_health <= 0){
